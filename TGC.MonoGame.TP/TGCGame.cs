...
public class TGCGame : Game
{
    ...
    protected override void Initialize()
    {
        ...
        _camera = new FreeCamera(GraphicsDevice.Viewport.AspectRatio, new Vector3(0, 10, 50), screenCenter);
    ...
    protected override void Draw(GameTime gameTime)
    {
        ...
        var camPos = Matrix.Invert(_camera.View).Translation;
        var camText = string.Format("Camera: X={0:F2} Y={1:F2} Z={2:F2}\nF1: Switch Interior/Exterior", camPos.X, camPos.Y, camPos.Z);

        if (!_showInsideOnly)
        {
            _tilemap.Draw(_camera.View, _projection);
            _propmap.Draw(_camera.View, _projection);
        }
        else
        {
            _insidepropmap.DrawWireframe(GraphicsDevice, _camera.View, _projection);
        }
